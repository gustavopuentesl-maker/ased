...
# ══════════════════════════════════════════════════════════
    ...
    with fa2: blur=st.slider("Difuminado:",5,20,10)
    ...
    # Normalizar duración y clientes para ponderar (0-1)
    ...
    # ── Tres pestañas de mapas ─────────────────────────────────────────────
    tab1, tab2, tab3 = st.tabs([
        "Mapa 1 — Frecuencia de fallas",
        "Mapa 2 — Tiempo de desconexión",
        "Mapa 3 — Combinado (frecuencia + tiempo + clientes)",
    ])

    def construir_mapa():
        return folium.Map(location=[-34.98,-71.08],zoom_start=11,
                          tiles="CartoDB positron")

    # ── Mapa 1: Frecuencia (un punto por falla, sin peso) ─────────────────
    with tab1:
        st.markdown("**Zonas más rojas = mayor cantidad de fallas registradas**")
        m1=construir_mapa()
        pts1=[]
        for _,row in df_f.iterrows():
            ...
            pts1.append([la,lo])
        if pts1:
            HeatMap(pts1,radius=radio,blur=blur,min_opacity=0.3).add_to(m1)
    ...
    # ── Mapa 2: Tiempo de desconexión (peso = duración normalizada) ────────
    with tab2:
        st.markdown("**Zonas más rojas = mayor tiempo acumulado de interrupción (horas)**")
        m2=construir_mapa()
        pts2=[]
        for _,row in df_f.iterrows():
            if row["dur_norm"] > 0:
                la,lo=u2l(row["X_f"],row["Y_f"])
                pts2.append([la,lo,float(row["dur_norm"])])
        if pts2:
            HeatMap(pts2,radius=radio,blur=blur,min_opacity=0.3).add_to(m2)
        st_folium(m2,width=900,height=500,key="mapa2")
        # Estadísticas de duración
        d1,d2,d3=st.columns(3)
        d1.metric("Duración promedio total",
                  f"{df_f['Duracion'].mean():.2f} hrs")
        d2.metric("Duración máxima registrada",
                  f"{df_f['Duracion'].max():.2f} hrs")
        d3.metric("Duración total acumulada",
                  f"{df_f['Duracion'].sum():.1f} hrs")

        # Tabla top 5 zonas con más horas
    ...
    # ── Mapa 3: Combinado (frecuencia + duración + clientes afectados) ─────
    with tab3:
        st.markdown("""**Zonas más rojas = mayor impacto combinado:**
        frecuencia de fallas + horas de desconexión + clientes afectados""")
        m3=construir_mapa()
        pts3=[]
        for _,row in df_f.iterrows():
            la,lo=u2l(row["X_f"],row["Y_f"])
            pts3.append([la,lo,float(row["peso_total"])])
        if pts3:
            HeatMap(pts3,radius=radio,blur=blur,min_opacity=0.3).add_to(m3)
        st_folium(m3,width=900,height=500,key="mapa3")
        # Estadísticas combinadas
        e1,e2,e3,e4=st.columns(4)
        e1.metric("Total fallas",f"{len(df_f):,}")
        e2.metric("Horas totales acumuladas",f"{df_f['Duracion'].sum():.1f}")
        e3.metric("Total clientes afectados",f"{df_f['Clientes'].sum():.0f}")
        e4.metric("Promedio clientes/falla",f"{df_f['Clientes'].mean():.1f}")

        # Tabla resumen por tipo
        st.markdown("**Resumen por tipo de falla:**")
        resumen=df_f.groupby("Calificacion").agg(
            Fallas=("INC","count"),
            Horas_prom=("Duracion","mean"),
            Horas_total=("Duracion","sum"),
            Clientes_prom=("Clientes","mean"),
        ).round(2).reset_index()
        resumen.columns=["Tipo","N° Fallas","Hrs promedio","Hrs totales","Clientes promedio"]
